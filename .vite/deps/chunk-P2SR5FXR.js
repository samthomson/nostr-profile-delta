...
function getFilterLimit(filter) {
  ...
  return Math.min(
    ...
    ((_d = filter.authors) == null ? void 0 : _d.length) && ((_e = filter.kinds) == null ? void 0 : _e.every((kind) => isAddressableKind(kind))) && ((_f = filter["#d"]) == null ? void 0 : _f.length) ? filter.authors.length * filter.kinds.length * filter["#d"].length : Infinity
  );
}
  ...
  publish(relays, event, options) {
    return relays.map(normalizeURL).map(async (url, i2, arr) => {
      ...
      return r.publish(event).catch(async (err) => {
        if (err instanceof Error && err.message.startsWith("auth-required: ") && (options == null ? void 0 : options.onauth)) {
...
*/
//# sourceMappingURL=chunk-P2SR5FXR.js.map
